...
//                }
        ...
        string strInfoType = "";
        if(infoType == 0){strInfoType = "State";}
        if(infoType == 1){strInfoType = "Car Make";}
        if(infoType == 2){strInfoType = "Car Model";}

        printText("Enter Your "+strInfoType+":", false, width*16, 16 * height + 50 -75, &window, 20, true, true);
        printText(name + "|", false, width*16, 16 * height + 50 , &window, 18, false, true);
        window.display();
    }
    cout << name << endl;
    return name;
